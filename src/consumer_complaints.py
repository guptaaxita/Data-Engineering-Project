...
if sys.platform == 'win32':
    ...
    input_path = './input/'
    output_path = './output/'
    test_input_path = './insight_testsuite/test_input/'
    test_output_path = './insight_testsuite/test_output/'
...
def create_reports(input_path, output_path, input_filename, output_filename, flag = 'whole data'):
    ...
    for prod_yr in set(prod_yr_list):
        print(prod_yr[0], ', ', prod_yr[1], ', ',
              len(set([j for i, j in zip(prod_yr_list, list_company) if i == prod_yr])))
    print('\n')
